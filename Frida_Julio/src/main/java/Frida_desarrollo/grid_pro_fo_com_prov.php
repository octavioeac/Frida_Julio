...
{
	$objGrid->where("(estatus_proyecto_fo in ('POR ASIGNAR','POR ELABORAR','ASIGNADO') or estatus_proyecto_fo_b in ('POR ASIGNAR','POR ELABORAR','ASIGNADO')) 
	and (division like '%$sess_dd%' or division_b like '%$sess_dd%') 
	and (etapa_sisa<>'ECA' and etapa_sisa<>'CAN' and etapa_sisa<>'OBS' 
	and etapa_sisa<>'OPE' and etapa_sisa<>'BAJ')");
}elseif($perfil=="IP FO")
{
	$objGrid->where("(estatus_proyecto_fo in ('POR ASIGNAR','POR ELABORAR','ASIGNADO') or estatus_proyecto_fo_b in ('POR ASIGNAR','POR ELABORAR','ASIGNADO')) 
	and (division like '%$sess_dd%' or division_b like '%$sess_dd%') 
	and (etapa_sisa<>'ECA' and etapa_sisa<>'CAN' and etapa_sisa<>'OBS' 
	and etapa_sisa<>'OPE' and etapa_sisa<>'BAJ') and (ip_fibra_optica='$sess_nmb' or ip_fibra_optica_b='$sess_nmb')");
}
...
$objGrid -> FormatColumn("ip_fibra_optica", "IPE (LADO A)", 30, 30, 1, "180", "azul", "left");
...
$objGrid -> FormatColumn("ip_fibra_optica_b", "IPE (LADO B)", 30, 30, 1, "180", "azul", "left");
...
{ 
	if(estatus!="NO REQUERIDA" && estatus!="LIQUIDADA")
	{
		if (estatus=="POR ASIGNAR")	
			...
			{window.location.href='alta_construccion_fo_prov_ajax.php?ref_sisa_a='+ref_sisa+'&envia_punta=A&tabla='+tabla+'';}
	}
} 
...
{ 
	if(estatus!="NO REQUERIDA" && estatus!="LIQUIDADA")
	{
		if (estatus=="POR ASIGNAR")	
			...
			{window.location.href='alta_construccion_fo_prov.php?ref_sisa_a='+ref_sisa+'&envia_punta=B';}
