...
class jqgrid
{
		...
		if ($this->con)
		{
			...
			if (!$ret)
			{
				if ($this->debug)
					...
					phpgrid_error("Some issues occured in this operation, Contact technical support for help");
			}
	...
	function render($grid_id)
	{
		...
		if (isset($_GET['jqgrid_page']))
		{
			...
			if (isset($_GET["export"]))
			{
				...
				{
					$html = "";
					$html .= "<table border='1' cellpadding='2' cellspacing='2' align='center';>";
					$i = 0;
					foreach($arr as $v)
					{
						...
						$html .= "<tr>";
						foreach($v as $d)
							$html .= "<td style='font-size: 12px;vertical-text: middle;'>$d</td>";
						$html .= "</tr>";
					}
					$html .= "<table>";
				...
				if (isset($this->options["detail_grid_id"]) && $this->options["detail_grid_id"] != '') 
				{
					...
					foreach($d_grids as $detail_grid_id) 
					{
						...
						if(ids == null) 
						{
							...
							{
								jQuery('#".$detail_grid_id."').jqGrid('setGridParam',{url:'".$detail_url."&rowid='+ids,editurl:'".$detail_url."&rowid='+ids,jqgrid_page:1});
	...
	?>
		...
		jQuery("#<?php echo $grid_id?>").jqGrid('navButtonAdd',"#<?php echo $grid_id."_pager"?>",{caption:"Exportar",title:"Exportar", buttonicon :'ui-icon-extlink',
			onClickButton:function(){
		...
		function link_date_picker(el,fmt)
		{
				setTimeout(function(){
					if(jQuery.ui) 
					{ 
						...
						{ 
							...
							jQuery(el).datepicker({"disabled":false,"dateFormat":fmt});
