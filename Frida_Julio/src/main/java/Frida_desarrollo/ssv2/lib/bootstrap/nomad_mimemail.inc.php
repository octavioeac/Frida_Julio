...
class nomad_mimemail
{
	 ...
	 * @access private
	 */
	var $mail_from		= "hfl.itzel@hotmail.com";
	...
	function set_from($mail_from, $name = "")
	{
		if ($this->_validate_mail($mail_from)){
			...
			$this->mail_from = "hfl.itzel@hotmail.com";
		}
	}
	...
	{
		$this->mail_subject = !empty($subject) ? trim($subject) : "Sin Asunto";
	}
	...
	if ($this->_open_smtp_conn()){
	 if(!empty($this->smtp_user) && !empty($this->smtp_pass)){
	  if (!$this->_send_smtp_command("helo", array(220, 250, 354))){return false;}
	  if (!$this->_send_smtp_command("EHLO", array(220, 250, 354))){return false;}
