...
class jqgrid
{
	...
	var $select_command;
	// Nombre de la tabla de la bd utilizada para añadir, actualizar y borrar
	var $table;
	...
	var $debug;
	//identificador de conexión a la bd - no se utiliza ahora, @todo: por la necesidad de integrar adodb lib
	var $con;
	var $db_driver;
	...
	/**
	 *Contructor para configurar parámetros predeterminados
	 */
	function jqgrid($db_conf = null)
	{
		...
		if ($db_conf)
		{
			...
			// Cambiar la codificación de la bd - para caracteres de ascenso (si es necesario) 
		...
		// Especificar el nombre de la columna para agrupar la lista
 
		// Mostrar datos ordenados dentro del grupo
		$grid["groupingView"]["groupDataSorted"] = array(true);  
		// Trabajar con summaryType, summaryTpl, vea la columna: $ col ["name"] = "total"; 
		$grid["groupingView"]["groupSummary"] = array(true);  
		// Cambiar a true para mostrar grupo colapsado (default: false) 
		$grid["groupingView"]["groupCollapse"] = false;  
		// Mostrar fila de resumen, incluso si el grupo se colapso (ocultar) 
		...
		// Pasar parámetros al subgrid si existe
		$s = (strstr($grid["url"], "?")) ? "&":"?";
		...
		// Si se solicita una exportación especifica
		...
		// Opciones de exportación PDF predeterminados
	...
	/**
	 * Función de ayuda para analizar arreglo
	 */
	...
	private function construct_where($s)
	{
		...
		if ($s) {
			...
			if(is_array($jsona))
			{
				...
				{
					# Nombres de los campos de la tabla para solucionar conflictos (alias utilizado en la página, propiedad dbname)
	...
	/**
	 * Obtener valores desplegables para seleccionar menús desplegables
	 */	
	...
	/**
	 * Setter para permitir acciones (añadir/editar/borrar/autofiltro etc)
	 */
	...
	/**
	 * Setter para el las opciones de personalización del grid
	 */
	function set_options($options)
	{
		...
		// Para la exportación como matriz de fusión
	...
	/**
	 * Generar automáticamente las columnas de cuadrícula
	 */
	function set_columns($cols = null)
	{
		...
		// Si sólo está definida la tabla, seleccione SQL para hacerlo
		if (!$this->select_command && $this->table)
			$this->select_command = "SELECT * FROM ".$this->table;

		// Anadir clausula where si no esta presente -- fijar a la función de búsqueda
		if (stristr($this->select_command,"WHERE") === false)
		{
			// Colocar por grupo en la posición adecuada en sql
		...
		// Hacer una línea simple en sql con espacios extra
	...
	// +++++
		...
		// Indice attr es imprescindible para jqGrid, para añadir en la matriz
	...
	/**
	 * Función común para las operaciones en la bd
	 */
	function execute_query($sql,$return="")
	{
		if ($this->con)
		{
			...
			{
				if ($this->debug)
					phpgrid_error("No se pudó ejecutar la consulta. ".$this->con->ErrorMsg()." - $sql");
				else
					phpgrid_error("Algunos problemas ocurridos en esta operación, Contacte al soporte técnico para obtener ayuda.");
			}
		...
		else
		{
			...
			{
				if ($this->debug)
					phpgrid_error("No se pudó ejecutar la consulta.".mysql_error()." - $sql");		
				else
					phpgrid_error("Algunos problemas ocurridos en esta operación, Contacte al soporte técnico para obtener ayuda.");			
			}
	...
	/**
   	 * Generar matriz JSON para la renderización del grid
	 * @ Param $ grid_id ID único para la red
	 */
			...
			if(!empty($_FILES[$fileElementName]['error']))
			{
				switch($_FILES[$fileElementName]['error'])
				{
					case '1':
						...
						break;
					case '2':
						$error = 'El archivo subido excede la directiva MAX_FILE_SIZE que se especificó en el formulario HTML';
						break;
					case '3':
					...
					case '7':
						...
						break;
					case '8':
						$error = 'Carga de archivos detenido por extensión';
						break;
					case '999':
					default:
...
// $g->select_command = "SELECT id, nombre, anio,enero,febrero,marzo,abril,mayo,junio,julio,agosto,septiembre,octubre,noviembre,diciembre FROM tabla_graficas"; 
...
// $consulta=$g->select_command = $consulta; 
