...
function central($div,$plan){
    $centrales = array();
    $inx = $plan == 'Plan 2014' ? 1 : 0;
    $query = array(
        "SELECT centrales.id_ctl,CONCAT(centrales.edificio,' - ',centrales.sigcent) ec FROM centrales,cat_programa_vdsl WHERE centrales.dir_div='".$div."' AND centrales.dir_div=cat_programa_vdsl.division AND centrales.sigcent=cat_programa_vdsl.siglas ORDER BY centrales.edificio ASC",
        "SELECT id_ctl,CONCAT(edificio,' - ',sigcent) ec FROM centrales WHERE dir_div='".$div."' ORDER BY edificio ASC"
    );
    $result = mysql_query($query[$inx]);
...
function proveedor($rubro,$plan){
    $proveedor = array();
    $seq = subquery($plan);
    $query = "SELECT proveedor from ztecnologias WHERE rubro='".$rubro."'".$seq." GROUP BY proveedor";
    $result = mysql_query($query);
    $sz = mysql_num_rows($result);
    if($sz > 0){
        for($i = 0; $i < $sz; $i++){
