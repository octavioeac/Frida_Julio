var limparAcceso = function(r){
    $('#rubro').find('option').removeAttr('disabled');
...
var types = function(i,j){
    var t = [
        ...
        $('<input>',{type:'text',name:'puertos'+i,disabled:'disabled',autocomplete:'off',placeholder:'Puertos'}),
        $('<input>',{type:'text',name:'tarjetas'+i,disabled:'disabled',autocomplete:'off',placeholder:'Tarjetas'}),
...
$(function(){
    ...
    //  VARIABLES GENERALES
        var plan,division,central,rubro,proveedor,tecnologia,modelo,claseRepisa,codTjt,noequipos=0,xobj,pxt,tjm,ttl;
        var d, a, p, v, r, t, nextinput = 0,nextinput2 = 0, campo;
        
    var rsu = $('input[name=usr]').val();
    if(rsu == 'olazcano'){
    ...
    $('#plan').change(function(){
        ...
        if(plan != ''){
            if(plan == 'Plan FTTH'){
                $('#rubro').find('option').eq(2).attr('disabled','disabled');
            }
    ...
    //LLENADO COMBO PROVEEDOR INFRAESTRUCTURA
    $.ajax({
        data:{provinfra:'provinfra'},
        success:function(data){
         $('#noprov_i').append($('<option>',{value:'',text:'Seleccionar'}));
         $('#noprov_ifo').append($('<option>',{value:'',text:'Seleccionar'}));
             $.each(data,function(i,v) {
                    $('#noprov_i').append($('<option>',{value:v,text:v}));
                    $('#noprov_ifo').append($('<option>',{value:v,text:v}));
                });
        }
    });
    ...
    $('fieldset.nu').on('change','#claseRepisa',function(){
        ...
        if(claseRepisa != ''){
            $('fieldset.nu:eq(1),fieldset.nu:eq(2)').remove();
    ...
    $('fieldset.nu').on('change','#codigoTarjeta',function(){
        var code = $(this).val();
        if(code != ''){
            $('input[name^=puertos],input[name^=tarjetas]').removeAttr('disabled');
            $('#tipoTarjeta').empty();
            ...
            $.ajax({
               data:{rubro_:rubro,proveedor_:proveedor,tecnologia_:tecnologia,tipoEquipo_:modelo,claseRepisa_:claseRepisa,codTarjeta_:code},
               success:function(data){
                   pxt = parseInt(data[0]);
                   tjm = parseInt(data[1]);
                   ttl = parseInt(data[2]);
        ...
        if(rubro != ''){
            ...
            if(rubro == 'ACCESO'){
                ...
                $('fieldset.nu:eq(0)').append(
                    $('<div>').append(
                        $('<label>',{text:'Modelo:'}),
                        $('<select>',{name:'modelo',id:'modelo',disabled:'disabled', class:'validate[required]'})
                    ).addClass('two'),
                    $('<div>').append(
                        $('<label>',{text:'Clase de Repisa:'}),
                        $('<select>',{name:'claseRepisa',id:'claseRepisa',disabled:'disabled',class:'validate[required]'})
                    ).addClass('two'),
                    $('<div>').append(
                        $('<label>',{text:'Código de Tarjeta:'}),
                        $('<select>',{name:'codigoTarjeta',id:'codigoTarjeta',disabled:'disabled',class:'validate[required]'})
                    ).addClass('two'),
                    $('<div>').append(
            ...
            else if(rubro == 'TRANSPORTE' || rubro == 'PROCESAMIENTO'){
            ...
            $.ajax({
                ...
                success:function(data){
                    ...
                    $('ul.'+prevul).append(
                        ...
                        $('<li>').append($('<input>',{type:'text',name:'puertos'+obj[prevul][0]+total,disabled:'disabled',autocomplete:'off',placeholder:'Puertos'})).addClass('mn yel'+clase),
                        $('<li>').append($('<input>',{type:'text',name:'tarjetas'+obj[prevul][0]+total,maxlength:'3',disabled:'disabled',autocomplete:'off',placeholder:'Tarjetas'})).addClass('mn yel'+clase),
    ...
    $('#agregarPedido').click(function(){
        nextinput2++;
        campo = '<li id="rut'+nextinput2+'"><label class="large">Pedido <span class="x">*</span></label><input type="text" id="pedido'+nextinput2+'" name="pedido'+nextinput2+'" class = "validate[required,custom[pedido]]" /><div class="close" onclick="del('+nextinput2+');"></div></li>';
        $("#camposPedido ul").append(campo);  
        $('#pedidos').val(nextinput2);     
        return false;
    });
    ...
    $('fieldset.nu:eq(0)').on('keyup','input[name^=puertos]',function(){
        ...
        if(!n){
            ...
            pto > ttl ? $('input[name=puertos'+index+'],input[name=tarjetas'+index+']').val('') : $('input[name=tarjetas'+index+']').val(Math.ceil(pto/pxt));
        }
    });
    $('fieldset.nu:eq(0)').on('keyup','input[name^=tarjetas]',function(){
        var pto,regx,n,index;
        pto = parseInt($(this).val());
        regx = /^[\d]{1,2}$/;
        n = regx.test(pto);
        ...
        if(!n){
            ...
            pto > tjm ? $('input[name=puertos'+index+'],input[name=tarjetas'+index+']').val('') : $('input[name=puertos'+index+']').val(Math.ceil(pto * pxt));
        }
    });
    ...
    $('form#winone').on('keyup','fieldset.nu:eq(1) input[name^=puertos],fieldset.nu:eq(2) input[name^=puertos]',function(){
        var id,ind,regx,eva,tjt,integ;
        tjt = $(this).val();
        integ = parseInt(tjt);
        ind = ($(this).attr('name')).substring(7);
        ...
        if(!eva){
            ...
            integ > (xobj[id][3]*xobj[id][4]) ? $('input[name=puertos'+ind+'],input[name=tarjetas'+ind+']').val('') : $('input[name=tarjetas'+ind+']').val(Math.ceil(integ/xobj[id][3]));
        }
    });
    
    $('form#winone').on('blur','fieldset.nu:eq(1) input[name^=puertos],fieldset.nu:eq(2) input[name^=puertos]',function(){
        var c = $(this).val();
        var ind = ($(this).attr('name')).substring(7);
        var id = $('select[name=nom'+ind+'] option:selected').attr('id');
        var m = Math.ceil(c/pxt);
        c = xobj[id][3] * m;
        $(this).val(c);
    });
    
    $('form#winone').on('keyup','fieldset.nu:eq(1) input[name^=tarjetas],fieldset.nu:eq(2) input[name^=tarjetas]',function(){
        var id,ind,regx,eva,tjt,integ;
        tjt = $(this).val();
        integ = parseInt(tjt);
    ...
    $('form#winone').on('change','select[name^=nom]',function(){
        ...
        $('input[name=puertos'+id+'],input[name=tarjetas'+id+']').removeAttr('disabled');
    });
});