...
function pop_up_dialog(pop_up,name_id,funcion_name,json_stri,ref_sisa,punta,tabla){
   ...
   /* var x=window.open('', '', 'titlebar=no,scrollbars=no,,location =no,menubar=no,height='+100+',width='+100+',resizable=no,toolbar=no,location=no,status=no,left='+100+',top='+100+'');
	 ...
	 buttons: {
	       "Actualiza" : function(){ 
		   try{
		   var input_text =document.formEditName.suger_nodo.value;
		  var texto =document.formEditName.observaciones.value;
		   envia_ajax(funcion_name,json_stri,ref_sisa,punta,tabla,texto,input_text);
		  			
	 //  jq2( this ).dialog( "close" );
      jq2(this).dialog('destroy').remove();
	  }
	  	catch(err){
		var hola=getElementById('suger_nodo');
		alert(hola);
		     
			 var $this = $(this);
			 var dialog = $this.find('#formEditName');
		   alert(dialog);
		}
	     	},
           Cancel: function() {
