		...
		if ($eq_tdemar=='DDE' || $eq_tdemar=='DDE-N' || $eq_uso_equipo=='CLIENTE') // dde / dde-n = cliente  // cliente 
			{ //echo "<br>___CLIENTE___<br><br>"; 
			   ...
			   $var_condicion="nombre_oficial_pisa";
				...
				$var_Sem_cte = " clli_adva='".$clli_cte."' AND nomof LIKE '%".$ref_sisa_a."%' "; 
			}
				...
				if ($i==0)
					{ // CLIENTE
						if ($uso_puerto_a=='CLIENTE' || $uso_puerto_a=='CLIENTE-PRIMARIO')
											{$puerto_tx=$puerto_a; $tipo_puerto_tx=$tipo_puerto_a; $aplica_puerto_a1='OK';}
...
for ($i=0; $i<count($var_Sem_A); $i++)
	{
		if ($var_Sem_A[$i]!='') 
			{
				$querySemPDe = "SELECT estatus_alta, estatus_ap, estatus_val_sup, estatus_proveedor, estatus_cm, estatus_cna, estatus_ptoext, estatus_adva, estatus_adva_exp, nomof FROM inventario_demarcadores WHERE ".$var_Sem_A[$i]."";
				...
				for ($j=0; $j<$variablerowSem; $j++)
					{
						...
						$estatus_cna_S[] = mysql_result($resSemPDe,$j,'estatus_cna'); 				
						$estatus_adva_S[] = mysql_result($resSemPDe,$j,'estatus_adva');				$estatus_adva_exp[] = mysql_result($resSemPDe,$j,'estatus_adva_exp');
						$estatus_ptoext_S[] = mysql_result($resSemPDe,$j,'estatus_ptoext');
					}
			}
	}
...
$color=array(
				"POR VALIDAR"=>'#FFFFB7', "POR REVISAR"=>'#FFFFB7', "VALIDADA"=>'#A5DF00', "ASIGNACION DE TECNICO"=>'#FFFFB7', "AUTORIZADA"=>'#FFFFB7',
				...
				"EJECUTADA SIN EXITO"=>'#FE2E2E', "RECHAZADA"=>'#FE2E2E', 
			);
?>
...
<table class="tbGeneralBl">
	...
	$color_cole[9]=$color[$estatus_adva_exp[2]];		
		 	
			//===============Marca en verde estatus anteriores
			/*for($p=0;$p<9;$p++){
	...
	elseif($rowSL['tabla']=='ciudad_segura'){	?>	
		<td bgcolor="<?=$color_cole[0]?>" 	title="<?=$estatus_alta_S[2]?>">PROYECTO </td>
		<td bgcolor="<?=$color_cole[9]?>" 	title="<?=$estatus_adva_exp[2]?>">VALIDACION EXPLOTACION</td>
		<td bgcolor="<?=$color_cole[6]?>" 	title="<?=$estatus_ptoext_S[2]?>">CONFIG TX &nbsp;&nbsp;(CNS I)</td>
		<td bgcolor="<?=$color_cole[7]?>" 	title="<?=$estatus_adva_S[2]?>">CONFIG ACCESO (CNS IV)</td>
	<?php
	}
...
<table id="tbGeneral">
	...
	<tr>
		...
		<td colspan="2"><input name="text6" type="text" id="text6" value="<?=$estatus_cole?>" size="30" readonly='readonly' /><?php if($rowSL['tabla']=='ciudad_segura'){?>
			<input type="button" name="Submit" value="Inventario" onclick="window.open('alta_equipo_cs.php?id_equipo=<?php echo $cole_id; ?>&refEnv=<?php echo  $_REQUEST['ref_sisa_a']; ?>&lado=<?php echo  $_REQUEST['envia_punta']; ?>&medioAcceso=<?php echo  $rowSL['material']; ?>')" />
		<?php } ?></td>
		<td></td>
	</tr>
...
<table class="tbGeneralBl">
	...
	$color_central[9]=$color[$estatus_adva_exp[0]];		
		 	
			//================Mostrar en verde estatus anteriores
			/*for($p=0;$p<9;$p++){
	...
	?>
		<td bgcolor="<?=$color_central[0]?>" 		title="<?=$estatus_alta_S[0]?>">PROYECTO </td>
		<td bgcolor="<?=$color_central[9]?>" 	title="<?=$estatus_adva_exp[0]?>">VALIDACION EXPLOTACION</td>
		<td bgcolor="<?=$color_central[6]?>" 	title="<?=$estatus_ptoext_S[0]?>">CONFIG TX &nbsp;&nbsp;(CNS I)</td>
		<td bgcolor="<?=$color_central[7]?>" 		title="<?=$estatus_adva_S[0]?>">CONFIG ACCESO (CNS IV)</td>
	<?php } ?>
	</tr>
...
<table id="tbGeneral">
	...
	<tr>
		...
		<td colspan="2">
			...
			<?php if($rowSL['tabla']=='ciudad_segura'){?>
			<input type="button" name="Submit" value="Inventario" onclick="window.open('alta_equipo_cs.php?id_equipo=<?php echo $ctrl_id; ?>&refEnv=<?php echo  $_REQUEST['ref_sisa_a']; ?>&lado=<?php echo  $_REQUEST['envia_punta']; ?>&medioAcceso=<?php echo  $rowSL['material']; ?>')" />
		<?php } ?>
		</td>
		<td></td>
	</tr>
...
<?PHP } ?> 
	...
	<?php
	//============Puertos
		$sql_puertos_id = "SELECT puerto,ubicacion_bdfo,repisa_bdfo,contacto_bdfo FROM inventario_puertos_demarcadores WHERE clli_adva='".$GE_clli_ctrl."' AND nombre_oficial_pisa='".$GE_clli_cte."' AND nomof LIKE '%".$_REQUEST['ref_sisa_a']."%'   ";
	...
	<tr class="tituloRojo1"> <td colspan="4" align="left" bordercolor="#999999">Puertos</td> </tr>
			...
			<?php
				...
				{
					echo "<tr><td bordercolor='#999999'><input type='text' value='".mysql_result($sql_puertos_cobre,$i,0)."' readonly='readonly' size='40'></td>";
...
<?php 
	...
	$color_cliente[9]=$color[$estatus_adva_exp[1]];	
...
if($rowSL['tabla']!='ciudad_segura'){ ?>
	...
	?>
		<td bgcolor="<?=$color_cliente[0]?>" 	title="<?=$estatus_alta_S[1]?>">PROYECTO </td>
		<td bgcolor="<?=$color_cliente[9]?>" 	title="<?=$estatus_adva_exp[1]?>">VALIDACION EXPLOTACION</td>
		<td bgcolor="<?=$color_cliente[6]?>" 	title="<?=$estatus_ptoext_S[1]?>">CONFIG TX &nbsp;&nbsp;(CNS I)</td>
		<td bgcolor="<?=$color_cliente[7]?>" 	title="<?=$estatus_adva_S[1]?>">CONFIG ACCESO (CNS IV)</td>
	<?php } ?>
	</tr>
...
<table id="tbGeneral2">
	...
	<tr>
		...
		<td colspan="2">
			...
			if($rowSL['tabla']=='ciudad_segura' )
			{
				if($cte_id=='' &&  $rowSL['material']=='COBRE')
				{
			?>
					<input type="button" name="Submit" value="Alta" onclick="window.open('alta_eq.php?refEnv=<?php echo  $_REQUEST['ref_sisa_a'];?>&lado=<?php echo  $_REQUEST['envia_punta'];?>')" />
		<?php 
				}
				else
				{
		?>
					<input type="button" name="Submit" value="Inventario" onclick="window.open('alta_equipo_cs.php?id_equipo=<?php echo $cte_id; ?>&refEnv=<?php echo  $_REQUEST['ref_sisa_a']; ?>&lado=<?php echo  $_REQUEST['envia_punta']; ?>&medioAcceso=<?php echo  $rowSL['material']; ?>')" />
		<?php
				}
		     } ?>
		</td>
	...
	<?php } ?>
	<?php
	$sql_puertos_id = "SELECT puerto,ubicacion_bdfo,repisa_bdfo,contacto_bdfo FROM inventario_puertos_demarcadores WHERE clli_adva='".$GE_clli_cte."' AND nomof LIKE '%".$_REQUEST['ref_sisa_a']."%'";	
	$sql_puertos_cobre=mysql_query($sql_puertos_id);
	if(mysql_num_rows($sql_puertos_cobre)>0){
	?>
	<tr bgcolor="#999999"><td colspan="4" height="4"></td></tr>
